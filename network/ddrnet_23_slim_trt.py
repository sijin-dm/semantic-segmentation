...
from network.mynn import Norm2d as BatchNorm2d, scale_as
...
class DualResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = x_ + F.interpolate(self.spp(self.layer5(self.relu(x))),
                               size=[height_output, width_output],
                               mode='bilinear',
                               align_corners=align_corners)
        if self.is_backbone:
        ...
        x_ = scale_as(x_, inputs)

        if self.augment:
            x_extra = self.seghead_extra(temp)
            x_extra = scale_as(x_extra, inputs)
...
def DualResNet_imagenet(num_classes=19,
    ...
    if os.path.isfile(cfg.MODEL.DDRNET23_SLIM_CHECKPOINT):
        ...
        pretrained_state = torch.load(cfg.MODEL.DDRNET23_SLIM_CHECKPOINT,
                                      map_location='cpu')
...
def get_seg_model():
    return DualResNet_imagenet(is_backbone=True)
